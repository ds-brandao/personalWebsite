...
- **Tracked links**: `/r/<slug>` (targets in `lib/links.ts`) 302s to the destination and emails a click alert through Resend. Needs `RESEND_API_KEY`, `LINK_ALERT_TO` and `LINK_ALERT_FROM`: set them as GitHub repo secrets (the deploy workflow copies them onto the Worker on each push to `main`), or in `.env.local` for `npm run dev`. Without them the redirect still works and a warning is logged. The `LINK_ALERT_FROM` address must be on a domain verified in Resend.
